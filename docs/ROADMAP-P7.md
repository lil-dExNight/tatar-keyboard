...
## P7-7 — контракт без автопробела + длинный след с затуханием (2026-09-25)

### Изменение контракта (решение пользователя): глайд не ставит автопробел

«При наборе свайпом сразу после слова ставится пробел — такое не нужно, это не набор по
подсказке». Новый контракт:

- Подъём глайда вставляет слово БЕЗ хвостового пробела.
- Цепочка: если курсор сразу после словесного символа — перед словом вставляется ОДИН пробел
  (глайд за глайдом → «сәләм дөнья»); после пробела/пунктуации/на старте поля — ничего не
  добавляется. Реализация: `InputLogic.commitGlideWord(..., chainedAfter)` — единственное
  терпимое хвостовое слово — предыдущий коммит цепочки (в своём undo-окне); любое другое
  хвостовое слово — недопечатанный префикс, жест отклоняется fail-closed (гейт в
  `onGlideInput`: `trailingWord.isNotEmpty() && trailingWord != glideCommittedWord`).
  Возврат — `GLIDE_COMMIT_REFUSED/BARE/PREPENDED`: флаг prepend нужен undo.
- Альтернативы заменяют слово НА МЕСТЕ, пробелы не трогаются (суффикс = (space?)+слово,
  позиция: trailing word IS the committed word).
- Undo: один backspace удаляет слово ВКЛЮЧАЯ prepend-пробел («сәләм дөнья» → «сәләм»).
- После коммита хвостовое слово редактора — само скоммиченное слово: полоса дальше ведёт
  себя как при печатном слове (prefix-путь, формы) — пин: после тапа альтернативы полоса =
  формы «сәләм» (`сәләмәтлек · сәләмәт · сәләмә`; до P7-7 была NEXT_WORD-цепочка
  `сәләмә · һәм · белән` — старое→новое зафиксировано здесь).

Прогнозные пути (тап по подсказке, предсказание) автопробел СОХРАНЯЮТ — запинено в
контрактном тесте (`suggestion + AUTO_SPACE` живёт в commitPredictedWord).

### След: длиннее и с затуханием после подъёма

- Окно 150→300 мс, кольцо 48→96 точек (`GlideTrail.TAIL_MS/DEFAULT_CAPACITY`).
- Fade-out после подъёма 250 мс: `startFadeOut` замораживает кольцо, `fadeFactor(now)` гасит
  альфу линейно; вью (`MainKeyboardView.drawGlideTrail`) перевызывает себя
  `postInvalidateDelayed(16)` пока `isFadeDone` не остановит; первый же point нового жеста
  сбрасывает кольцо (без мостика от старого следа), detach окна чистит след. Цвет/толщина не
  тронуты (accent, 9dp).

### Пины

- `GlideEndToEndTest` (22): обновлены все auto-space утверждения (старое→новое: «сәлләм » →
  «сәлләм» и т.д.); новые: цепочка с одним prepend-пробелом, undo шага цепочки, глайд после
  напечатанного пробела ничего не добавляет, цепочка через альтернативу; распад альтернатив
  при печати переписан на инертность тапа (полоса форм с пустым результатом не перерисовывает).
- `GlideTrailTest` (11): окно/рампа пересчитаны на 300 мс; новые: затухание 1→0 за FADE_OUT_MS,
  пустой след не затухает, новый жест сбрасывает кольцо, zero-alloc и на fade-кадрах.
- Контракты: `GlideTouchIntegrationContractTest` — сигнатуры seam'ов, chain-гейт, отсутствие
  `suggestion + AUTO_SPACE` в глайд-пути; `GlideTrailContractTest` — startFadeOut/fadeFactor/
  isFadeDone/postInvalidateDelayed/detach-clear.
- androidTest `GlideUiDeviceTest` — лайв-поток: подъём без пробела, замена на месте, цепочка,
  undo цепочки (д-о-н-ь-я путь; на синтетических сегментах декод даёт «донья» — контракт
  тестирует разделители, не ранжирование).

### Гейты (финальное дерево)

- JVM: **1580/1580** (`test --rerun-tasks`); python-конвейер зелёный; lintRelease зелёный;
  rebuild_assets --check ok; release APK **1 846 564 Б**; check-no-internet — оба уровня.

### Device UAT (POCO C71, 2026-09-25)

Свидетельства: `build/device-uat-2026-09-25/p7-7/` (`u00-readbacks.txt` — все readback'и,
u01–u08). Отдельная заметка: устройство лежало в ландшафте и вендорский оверлей секундами
возвращал accelerometer_rotation=1 — навигация шла с периодическими локами; попутно доказан
fail-closed отказ глайда над недопечатанным словом. Состояние восстановлено: сборка
пользователя (HEAD 44befc7a), дефолтный IME наш, тумблеры не трогались (все ON как было),
accelerometer_rotation=1, debug-пакеты удалены.

## P7-8 — удвоенная буква требует улики в пути (2026-09-25)

### Репорт и корень

Пользователь (устройство): свайп «сәләм» коммитит «сәлләм» — удвоенное слово выигрывает по
частоте, а обычное сидит в полосе альтернатив. Корень (подтверждён чтением
`GlideDecoder.scoreCandidate`): слово с удвоенной буквой скорилось ЛУЧШИМ из двух вариантов
идеального пути (plain/looped) — а plain-вариант удвоенного слова вырождается в путь
неудвоенного близнеца (удвоенная буква даёт нулевой сегмент, который ресэмплер пропускает), так
что no-loop путь пользователя матчил сәлләм-plain ровно как сәләм, и решала частота.

### Правило

Удвоенная буква обязана иметь УЛИКУ в пути (джог/петлю у удвоенной клавиши): кандидат с
удвоением скорится ТОЛЬКО по looped-варианту идеального пути (`scoreCandidate` больше не делает
best-of-variants — выделен `scoreVariant`, вызывается один раз; P7-4 fused-walk не тронут).
Нет улики → удвоенный кандидат проигрывает (или выпадает). Прунинг по длине оставлен
разрешительным (plain OR looped) — скоринг понижает, отрезать не надо.

### Калибровка (held-out, реальный словарь; старый сет → новый сет)

Генератор (`scripts/glide_pack.py`): слово с удвоением теперь даёт ДВЕ строки сета — no-jog и
jog из одного слово-потока (пара отличается ровно детуром). Попутно измерено и записано:
старая модель рисовала петлю для 75.1 % удвоенных слов при номинале 1/8 (младшие биты
SplitMix64-стрима для модуля 8 недостаточно равномерны — с двухрядной моделью вопрос снят).
SHA-256 сета перепинен с двух сторон (4498 слов → 4928 строк; 9 181 165 → 10 113 092 Б).

Старый декодер → новый (held-out, по классам):

| класс | было | стало |
|---|---|---|
| plain (n=2080) top1 / top3 | 69.23 / 76.88 % | **69.62 / 77.16 %** (улучшение: удвоенные перестали красть победы) |
| doubled+jog (n=229) top1 / top3 | 77.27 / 84.09 % | 77.29 / 84.72 % (держится) |
| doubled+no-jog, близнец в словаре (n=28) top1 | ~79 % (старый класс no-jog) | **7.14 %** — правило работает |
| doubled+no-jog, без близнеца (n=201) top1 / top3 | — | 55.72 / 75.62 % (удвоенное — единственный кандидат формы, выигрывает честно) |

Толерансы записаны в тесте: plain-класс — регрессия ≤ 1.0 пп (измерено: +0.38 пп); twin-nojog
top-1 ≤ 10 %; jog top-1 ≥ 70 %. Общий held-out top-1 70.07 → 68.52 % — структурный сдвиг: сет
теперь вчетверо больше меряет класс, где «восстановление» удвоенного слова и был багом.

### Пины

- `GlideDecoderTest.aDoubledWordNeedsLoopEvidenceInThePath` — оба направления на одной
  инвентаризации (сәләм 36 vs сәлләм 7466): no-loop → сәләм, джог у л → сәлләм.
- `GlideRecoveryCalibrationTest` — per-class метрики + толерансы; сет-идентичность перепинена.
- e2e/UiDevice: top-1 сәләм-пути теперь «сәләм» (пины перевернуты), сәлләм в альтернативах.

### Device UAT (POCO C71, 2026-09-25)

- Релиз `/tmp/tt-glide6.apk` (sha256 7c5c9399…): no-jog полилиния с-ә-л-ә-м через
  `input motionevent` (on-device скрипт — по одному adb на точку слишком медленно) → поле
  читается «сәләм»; скриншот полосы альтернатив: сәлләм · сәдәп · сайлыйм. Свидетельства:
  `build/device-uat-2026-09-25/p7-8/`.
- Debug-инструментация (та же кодовая база): `GlideUiDeviceTest` — no-jog → «сәләм», тап
  левой ячейки → «сәлләм», цепочка, undo; новый
  `testALoopAtTheDoubledLetterDeliversTheDoubledWord` — джог-путь → «сәлләм».
- Регрессии: immediate-swipe и hold-then-swipe (GlidePointerDeviceTest 4/4), след не тронут.
- Заметка о среде: телефон лежал в ландшафте, вендорский оверлей секундами возвращал
  auto-rotate — часть ранних попыток UAT билась об это (не о продукт). Восстановление: сборка
  пользователя (`dist/tatar-keyboard-3.0.2.apk`, релиз 3.0.2) поставлена обратно, дефолтный
  IME наш, debug-пакеты удалены.
