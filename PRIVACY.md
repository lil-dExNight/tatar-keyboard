...
- **How to see and erase it.** Keyboard settings → **“Saved words”**: every saved word of every language, with **“Delete”** on each one and **“Erase everything saved.”** The same screen also lists the learned **word pairs** (see the next section), and the erase-everything action there covers both. Turning the personal dictionary off does **not** erase what was already saved — erase it here. Deleting the app, or clearing its data in the system settings, destroys everything saved, and no backup brings it back.
...
- **How to see and erase it.** Keyboard settings → **“Saved words”**: each language has a pairs card with every learned pair, **“Delete”** on each one and **“Clear all word pairs.”** The screen's erase-everything action destroys **both** the words and the pairs stores, their pending-hash files and salts. Deleting the app or clearing its data destroys everything.
