 ...
 */
class FocusStrategy {
    public static <L, T> T findNextFocusInRelativeDirection(L focusables,
            CollectionAdapter<L, T> collectionAdapter, BoundsAdapter<T> adapter,
            T focused, int direction, boolean isLayoutRtl,
            boolean wrap) {
    ...
    public static <L, T> T findNextFocusInAbsoluteDirection(L focusables,
            CollectionAdapter<L, T> collectionAdapter, BoundsAdapter<T> adapter,
            T focused, Rect focusedRect, int direction) {
    ...
    private static boolean isBetterCandidate(
            int direction, Rect source,
            Rect candidate, Rect currentBest) {
     ...
     */
    private static boolean beamBeats(int direction,
            Rect source, Rect rect1, Rect rect2) {
     ...
     */
    private static boolean isCandidate(Rect srcRect, Rect destRect,
            int direction) {
        switch (direction) {
            case View.FOCUS_LEFT:
     ...
     */
    private static boolean beamsOverlap(int direction,
     ...
     */
    private static boolean isToDirectionOf(int direction,
            Rect src, Rect dest) {
        switch (direction) {
            case View.FOCUS_LEFT:
     ...
     * returns 0.
     */
    private static int majorAxisDistance(int direction,
            Rect source, Rect dest) {
        return Math.max(0, majorAxisDistanceRaw(direction, source, dest));
    }

    private static int majorAxisDistanceRaw(int direction,
            Rect source, Rect dest) {
        switch (direction) {
            case View.FOCUS_LEFT:
     ...
     */
    private static int majorAxisDistanceToFarEdge(int direction,
            Rect source, Rect dest) {
     ...
     * @return the distance
     */
    private static int minorAxisDistance(int direction, Rect source,
            Rect dest) {
        switch (direction) {
            case View.FOCUS_LEFT:
            case View.FOCUS_RIGHT:
